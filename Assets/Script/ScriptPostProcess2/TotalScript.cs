...
public class TotalScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!check)
        {
            Invoke("MaxSalle", timeToReach+2f);
        }
        if (check)
        {
    ...
    void CreationDePorte()
    {
        ...
        for (int i = 0; i < porteR.Count; i++)
        {
            if (porteR[i].transform.childCount == 0 || porteR[i].transform.GetChild(0).gameObject.layer != LayerMask.NameToLayer("Wall"))
            {
                Instantiate(portePrefab, porteR[i].transform.position, Quaternion.identity, porteR[i].transform.parent.parent.GetChild(0).GetChild(0).GetChild(4));
            }
        }
        porteCheck = true;
